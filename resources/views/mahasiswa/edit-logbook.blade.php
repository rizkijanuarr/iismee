    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-28">
        <div class="bg-white shadow-2xl hover:shadow-[0_8px_30px_rgba(0,0,0,0.15)] transition-shadow rounded-2xl p-6 mb-4">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b pb-2 mb-2">
                <h2 class="text-2xl font-semibold text-gray-800 ">✏️ {{ __('messages.edit_logbook') }}</h2>
                <a href="{{ url('logbook') }}"
            ...
            <form action="{{ url('logbook/' . $logbook->id) }}" method="POST" enctype="multipart/form-data"
                id="editLogbookForm" class="space-y-4 bg-white ">
                @method('put')
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="activity_name" id="activity_name"
                        value="{{ old('activity_name', $logbook->activity_name) }}" placeholder="{{ __('messages.activity_name') }}"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm"
                        placeholder="{{ __('messages.write_activity_description') }}">{{ old('description', $logbook->description) }}</textarea>
                </div>
